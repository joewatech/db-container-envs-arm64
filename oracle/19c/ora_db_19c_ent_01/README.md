...
Docker Compose setup for a local Oracle Database 19c Enterprise Edition container.

## Usage

1. Create a `.env` file in this directory with `ORACLE_PDB_SID_PWD`, `ORACLE_PDB_LOCAL_PORT`, `ORACLE_PDB_LOCAL_OEM_PORT`, `ORACLE_PDB`, and `ORACLE_SID` (see `.env.example` and `compose.yaml` for how each is used). `.env` is gitignored and never committed.
